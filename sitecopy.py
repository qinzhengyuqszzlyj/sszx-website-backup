...
import requests, urllib, os, asyncio, functools, argparse, sys, shutil, subprocess
...
def start_download(url, depth, threads, entire, save_path):
    if entire:
        ExtractUrls(url, depth, threads)
    else:
        SaveSinglePage(url)
    shutil.copytree("website", save_path)
...
def start_gui():
    ...
    depth_label = ttk.Label(frame, text="深度:")
    depth_label.grid(row=1, column=0, sticky=tk.W)
    depth_entry = ttk.Entry(frame, width=10)
    depth_entry.grid(row=1, column=1, sticky=(tk.W, tk.E))
    depth_entry.insert(0, "200")

    threads_label = ttk.Label(frame, text="线程数:")
    threads_label.grid(row=2, column=0, sticky=tk.W)
    threads_entry = ttk.Entry(frame, width=10)
    threads_entry.grid(row=2, column=1, sticky=(tk.W, tk.E))
    threads_entry.insert(0, "30")

    entire_var = tk.BooleanVar()
    entire_check = ttk.Checkbutton(frame, text="下载整个网站", variable=entire_var)
    entire_check.grid(row=3, column=1, sticky=tk.W)

    save_path_label = ttk.Label(frame, text="保存路径:")
    save_path_label.grid(row=4, column=0, sticky=tk.W)
    save_path_entry = ttk.Entry(frame, width=50)
    save_path_entry.grid(row=4, column=1, sticky=(tk.W, tk.E))
    save_path_button = ttk.Button(frame, text="选择文件夹", command=lambda: select_folder(save_path_entry))
    save_path_button.grid(row=4, column=2, sticky=tk.W)

    start_button = ttk.Button(frame, text="开始下载", command=lambda: threading.Thread(target=start_download, args=(url_entry.get(), int(depth_entry.get()), int(threads_entry.get()), entire_var.get(), save_path_entry.get())).start())
    start_button.grid(row=5, column=1, sticky=tk.E)
...
def create_context_menu(entry):
    ...
    menu.add_command(label="删除", command=lambda: entry.event_generate("<<Clear>>"))
    menu.add_separator()
    menu.add_command(label="全选", command=lambda: entry.event_generate("<<SelectAll>>"))
    return menu

def on_context_menu(event, menu):
...
def start_gui():
    ...
    url_menu = create_context_menu(url_entry)
    depth_menu = create_context_menu(depth_entry)
    threads_menu = create_context_menu(threads_entry)
    save_path_menu = create_context_menu(save_path_entry)

    url_entry.bind("<Button-3>", lambda event: on_context_menu(event, url_menu))
    depth_entry.bind("<Button-3>", lambda event: on_context_menu(event, depth_menu))
    threads_entry.bind("<Button-3>", lambda event: on_context_menu(event, threads_menu))
    save_path_entry.bind("<Button-3>", lambda event: on_context_menu(event, save_path_menu))
